    ...
    def forward(self,extractFeatureMap):
        ...
        for l in range(self.K):
            if self.use_shortcut:
                ...
                propagatingFeature.append(self.upwardPass[l](propagatingFeature[-1]))
...
class GNNPlanner(nn.Module):
    ...
    def forward(self, x, test=False,tb=None):
        ...
        if tb is not None:
            ...
            fig=plt.figure()
            plt.imshow(x[2,0,:,:].detach().cpu().numpy())
            plt.colorbar()
            plt.savefig('cnn_input_0.png')
            fig=plt.figure()
            plt.imshow(x[2,1,:,:].detach().cpu().numpy())
            plt.colorbar()
            plt.savefig('cnn_input_1.png')
            fig=plt.figure()
            plt.imshow(x[2,2,:,:].detach().cpu().numpy())
            plt.colorbar()
            plt.savefig('cnn_input_2.png')
            fig=plt.figure()
            plt.imshow(x[2,3,:,:].detach().cpu().numpy())
            plt.savefig('cnn_input_3.png')
