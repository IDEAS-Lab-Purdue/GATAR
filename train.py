...
def get_state(instance,mrs):
    grid = instance.grid
    obs=mrs.observe(grid) #N*H*W*C
    obs=obs.permute(0,3,1,2) #N*C*H*W
    obs=obs.unsqueeze(0) #1*N*C*H*W->B*N*C*H*W
    
    adj_mat=mrs.get_adj_mat().unsqueeze(0).unsqueeze(0) #B*E*N*N
    SList=[adj_mat]
    for k in range(configs['network']['Fusion']['K']-1):
        adj_mat=adj_mat.matmul(adj_mat)
        SList.append(adj_mat)
    SList=torch.cat(SList,dim=1) #B*K*N*N
